/*-------------------------------------------------------------------------
  Particle Core, Particle Photon, P1, Electron, Argon, Boron, Xenon, B SoM, B5
  SoM, E SoM X, P2, Photon 2, Tracker and RedBear Duo library to control
...
#define pinHI(_pin) \
  (PIN_MAP[_pin].gpio_peripheral->BSRR = PIN_MAP[_pin].gpio_pin)
    ...
    (PLATFORM_ID ==                                                      \
     88)  // Photon (6), P1 (8), Electron (10) or Redbear Duo (88)
#if SYSTEM_VERSION < SYSTEM_VERSION_ALPHA(5, 0, 0, 2)
STM32_Pin_Info* PIN_MAP2 =
...
#elif HAL_PLATFORM_NRF52840  // Argon, Boron, Xenon, B SoM, B5 SoM, E SoM X,
                             // Tracker
#include "nrf.h"
#include "nrf_gpio.h"
...
#define pinLO(_pin)    \
  (nrf_gpio_pin_clear( \
      NRF_GPIO_PIN_MAP(PIN_MAP2[_pin].gpio_port, PIN_MAP2[_pin].gpio_pin)))
#define pinHI(_pin)  \
  (nrf_gpio_pin_set( \
      NRF_GPIO_PIN_MAP(PIN_MAP2[_pin].gpio_port, PIN_MAP2[_pin].gpio_pin)))
...
#else
#error \
    "*** PLATFORM_ID not supported by this library. PLATFORM should be Particle Core, Photon, Electron, Argon, Boron, Xenon, RedBear Duo, B SoM, B5 SoM, E SoM X, Tracker or P2 ***"
#endif
// fast pin access
...
void Adafruit_NeoPixel::begin(void) {
  ...
  if (getType() == WS2812B) {
    ...
    // spi_->begin(PIN_INVALID); // PIN_INVALID will keep begin from taking over
    // the default SS/SS1 pin as OUTPUT Note: no Wiring API yet to configure SPI
    // for MOSI ONLY
    ...
    hal_spi_begin_ext(spi_->interface(), SPI_MODE_MASTER, PIN_INVALID,
                      &spi_config);
    spi_->setClockSpeed(3125000);  // DVOS 5.7.0 requires setClockSpeed() to be
                                   // set after begin()
...
void Adafruit_NeoPixel::show(void) {
...
#if (PLATFORM_ID == 0) || (PLATFORM_ID == 6) || (PLATFORM_ID == 8) || \
    (PLATFORM_ID == 10) ||                                            \
    (PLATFORM_ID ==                                                   \
     88)  // Core (0), Photon (6), P1 (8), Electron (10) or Redbear Duo (88)
  __disable_irq();  // Need 100% focus on instruction timing

  volatile uint32_t c,             // 24-bit/32-bit pixel color
      mask;                        // 1-bit mask
  volatile uint16_t i = numBytes;  // Output loop counter
  volatile uint8_t j,              // 8-bit inner loop counter
  ...
  if (type == WS2812B ||
      type == WS2812B_FAST) {  // Same as WS2812 & WS2813, 800 KHz bitstream
    while (i) {                // While bytes left... (3 bytes = 1 pixel)
      ...
      c = ((uint32_t) g << 16) | ((uint32_t) r << 8) |
          b;  // Pack the next 3 bytes to keep timing tight
  ...
  } else if (type == SK6812RGBW) {  // similar to WS2812, 800 KHz bitstream but
                                    // with RGB+W components
    while (i) {                     // While bytes left... (4 bytes = 1 pixel)
      ...
      c = ((uint32_t) r << 24) | ((uint32_t) g << 16) | ((uint32_t) b << 8) |
          w;  // Pack the next 4 bytes to keep timing tight
  ...
  } else if (type == WS2812B2 ||
             type == WS2812B2_FAST) {  // WS2812B with DWT timer
#if (PLATFORM_ID == 6) || (PLATFORM_ID == 8) || (PLATFORM_ID == 10) || \
    (PLATFORM_ID ==                                                    \
     88)  // Photon (6), P1 (8), Electron (10) or Redbear Duo (88)
...
#define CYCLES_800_T1L 8   // 436ns (meas. 425ns)
  ...
  } else if (type == TM1803) {  // TM1803 (Radio Shack Tri-Color Strip), 400 KHz
                                // bitstream
    while (i) {                 // While bytes left... (3 bytes = 1 pixel)
      mask = 0x800000;          // reset the mask
      i = i - 3;                // decrement bytes remaining
    ...
    while (i) {         // While bytes left... (3 bytes = 1 pixel)
      ...
      b = *ptr++;       // Next blue byte value
      g = *ptr++;       // Next green byte value
      c = ((uint32_t) r << 16) | ((uint32_t) b << 8) |
          g;             // Pack the next 3 bytes to keep timing tight
      j = 0;             // reset the 24-bit counter
      pinSet(pin, LOW);  // LOW
      for (;;) {         // ... pixel done
        if (c & mask) {  // if masked bit is high
  ...
  constexpr uint8_t numBitsPerBit =
      3;  // How many SPI bits represent one neopixel bit
  ...
  for (int x = 0; x < numPixels(); x++) {
    for (int s = 0; s < 3; s++) {
      spiArray[(x * 9) + (s * 3) + 0 + resetOff] =
          ((0x80 & pixels[(x * 3) + s]) ? (PIX_HI << 5) : (PIX_LO << 5)) +
          ((0x40 & pixels[(x * 3) + s]) ? (PIX_HI << 2) : (PIX_LO << 2)) +
          ((0x20 & pixels[(x * 3) + s]) ? (0b11) : (0b10));
      spiArray[(x * 9) + (s * 3) + 1 + resetOff] =
          0 /* bit 7 always 0 */ +
          ((0x10 & pixels[(x * 3) + s]) ? (PIX_HI << 4) : (PIX_LO << 4)) +
          ((0x08 & pixels[(x * 3) + s]) ? (PIX_HI << 1) : (PIX_LO << 1)) +
          1 /* bit 0 always 1 */;
      spiArray[(x * 9) + (s * 3) + 2 + resetOff] =
          ((0x04 & pixels[(x * 3) + s]) ? (0b10 << 6) : (0b00 << 6)) +
          ((0x02 & pixels[(x * 3) + s]) ? (PIX_HI << 3) : (PIX_LO << 3)) +
          ((0x01 & pixels[(x * 3) + s]) ? (PIX_HI) : (PIX_LO));
    }
  }
...
#define CYCLES_400 156     // ~2.50 uS
  ...
  if ((pixels_pattern != NULL) && (pwm != NULL)) {
    ...
    pwm->PSEL.OUT[0] =
        NRF_GPIO_PIN_MAP(PIN_MAP2[pin].gpio_port, PIN_MAP2[pin].gpio_pin);
...
// Fall back to DWT
    ...
    uint32_t pinMask = 1UL << NRF_GPIO_PIN_MAP(PIN_MAP2[pin].gpio_port,
                                               PIN_MAP2[pin].gpio_pin);
  ...
  if (n < numLEDs) {
    ...
    switch (type) {
      ...
      case TM1829: {  // TM1829 is special RBG order
        if (r == 255)
          r = 254;  // 255 on RED channel causes display to be in a special
                    // mode.
        *p++ = r;
        *p++ = b;
        *p = g;
...
void Adafruit_NeoPixel::setColor(uint16_t aLedNumber,
                                 byte aRed,
                                 byte aGreen,
                                 byte aBlue) {
  return setPixelColor(aLedNumber, (uint8_t) aRed, (uint8_t) aGreen,
                       (uint8_t) aBlue);
}

void Adafruit_NeoPixel::setColor(
    uint16_t aLedNumber, byte aRed, byte aGreen, byte aBlue, byte aWhite) {
  return setPixelColor(aLedNumber, (uint8_t) aRed, (uint8_t) aGreen,
                       (uint8_t) aBlue, (uint8_t) aWhite);
}

void Adafruit_NeoPixel::setColorScaled(
    uint16_t aLedNumber, byte aRed, byte aGreen, byte aBlue, byte aScaling) {
  // scale RGB with a common brightness parameter
  setColor(aLedNumber, (aRed * aScaling) >> 8, (aGreen * aScaling) >> 8,
           (aBlue * aScaling) >> 8);
}

void Adafruit_NeoPixel::setColorScaled(uint16_t aLedNumber,
                                       byte aRed,
                                       byte aGreen,
                                       byte aBlue,
                                       byte aWhite,
  ...
  // scale RGB with a common brightness parameter
  setColor(aLedNumber, (aRed * aScaling) >> 8, (aGreen * aScaling) >> 8,
           (aBlue * aScaling) >> 8, (aWhite * aScaling) >> 8);
}

void Adafruit_NeoPixel::setColorDimmed(
...
void Adafruit_NeoPixel::setColorDimmed(uint16_t aLedNumber,
                                       byte aRed,
                                       byte aGreen,
                                       byte aBlue,
                                       byte aWhite,
                                       byte aBrightness) {
  setColorScaled(aLedNumber, aRed, aGreen, aBlue, aWhite,
                 brightnessToPWM(aBrightness));
}

byte Adafruit_NeoPixel::brightnessToPWM(byte aBrightness) {
  static const byte pwmLevels[16] = {0,  1,  2,  3,  4,  6,   8,   12,
                                     23, 36, 48, 70, 95, 135, 190, 255};
...
uint32_t Adafruit_NeoPixel::getPixelColor(uint16_t n) const {
  ...
  switch (type) {
    ...
    case SK6812RGBW: {  // SK6812RGBW is RGBW order, but returns packed WRGB
                        // color
