...
//
//               On Photon, Electron, P1, Core and Duo, any pin can be used for
//               Neopixel.
//
//               On the Argon, Boron and Xenon, only these pins can be used for
//               Neopixel:
//               - D2, D3, A4, A5
//               - D4, D6, D7, D8
//               - A0, A1, A2, A3
//
//               In addition on the Argon/Boron/Xenon, only one pin per group
//               can be used at a time. So it's OK to have one Adafruit_NeoPixel
//               instance on pin D2 and another one on pin A2, but it's not
//               possible to have one on pin A0 and another one on pin A1.
//
//               On Photon 2 / P2, only SPI(MOSI) or SPI1(D2) can be used for
//               Neopixel, and only PIXEL_TYPE's WS2812, WS2812B, WS2813 are
//               supported.
//               - MISO (D12), SCK (D13) and SS (D8) can be used as GPIO when
//               using SPI
//               - MISO1 (D3), SCK1 (D4) and SS1 (D5) can be used and GPIO when
//               using SPI1 note: You may want to call
//               System.disableFeature(FEATURE_ETHERNET_DETECTION);
//                     to disable automatic Ethernet driver detection, which
//                     will cause some glitches to SPI1 pins D2,D3,D4.
//
...
//               WS2812/WS2813 (6-pin part)/WS2812B (4-pin part)/SK6812RGBW
//               (RGB+W) )
//
